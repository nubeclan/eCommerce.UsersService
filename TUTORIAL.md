...
Para añadir nuevas funcionalidades REST, sigue el patrón existente descrito en `README.md`. Crea una nueva clase en la carpeta `Features` correspondiente (e.g., `Users` para usuarios).
...
### Ejemplo: Añadir Obtener Usuario por ID (Query)

#### 1. Crea el DTO de Respuesta
En `Contracts\Users\GetUserResponse.cs`:
```csharp
...
#### 2. Implementa la Query en `Features\Users\GetUser.cs`
```csharp
using Carter;
...
- Para autenticación, expande `Features\Auth` con JWT.
